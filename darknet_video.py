...
class DarknetVideo:
    ...
    def set_saved_video(self, input_video, output_video, size):
        ...
        fourcc = cv2.VideoWriter_fourcc(*"H264") #line was changed since streamlit can't display mp4v
    ...
    def video_capture(self, frame_queue, darknet_image_queue, cap):
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
                break
    ...
    def inference(self, darknet_image_queue, detections_queue, fps_queue, threshold, ext_output, cap):
        while cap.isOpened():
    ...
    def drawing(self, frame_queue, detections_queue, fps_queue, out_filename, cap):
        random.seed(3)  # deterministic bbox colors
        video = self.set_saved_video(cap, out_filename, (self.video_width, self.video_height))
        while cap.isOpened():
    ...
    def yolov4detect(self, config_file, data_file, weights, input, out_filename, threshold):
        frame_queue = Queue()
        ...
        threads.append(Thread(target=self.video_capture, args=(frame_queue, darknet_image_queue, cap)))
        threads.append(Thread(target=self.inference, args=(darknet_image_queue, detections_queue, fps_queue, threshold, True, cap)))
        threads.append(Thread(target=self.drawing, args=(frame_queue, detections_queue, fps_queue, out_filename, cap)))

        for t in threads:
            t.start()

        # wait for threads to finish processing the video before returning #
        for t in threads:
            t.join()
